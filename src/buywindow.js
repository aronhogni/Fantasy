/* ============================================================
   BUYWINDOW.JS — HVENAER A AD KAUPA ThENNAN LEIKMANN
...
export function buyWindows(series, { minLen = MIN_WINDOW, max = MAX_WINDOWS } = {}) {
  const known = series.filter(s => s.v != null);
  if (known.length < minLen) return { baseline: null, windows: [], n: known.length };
  ...
  const blocked = series.map(s => s.v == null);
  const windows = greedyWindows(series, baseline, minLen, max, blocked);
  return { baseline, windows, n: known.length };
}
...
function greedyWindows(rows, baseline, minLen, max, blocked) {
  const n = rows.length;
  const w = rows.map(r => (r.v == null ? -Infinity : r.v - baseline));
  ...
  for (let k = 0; k < max; k++) {
    ...
    out.push(makeWindow(rows.slice(best.a, best.b + 1), best.sum, best.score, baseline));
  }
...
function makeWindow(seg, gain, score, baseline) {
  ...
  return {
    ...
    perGw: +(gain / len).toFixed(2),
    /* UMFERDIR SEM MA BEKKJA — undir hans medaltali INNI i glugga. Thetta
       er thad sem notandinn kalladi „einn erfidur leikur sem haegt er ad
       bekkja". Auðar umferdir eru taldar SER: thar er ekkert ad bekkja. */
